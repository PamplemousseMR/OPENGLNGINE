...
void main()
{
    ...
    for(uint i=0 ; i<u_uiLightCount && i<MAX_LIGHT_COUNT ; ++i)
    {
        ...
        f3AmbientCol += u_f3Ambient * u_f3LightAmbient[i] * v_f4Color.xyz;
        f3DiffuseCol += u_f3Diffuse * u_f3LightDiffuse[i] * vec3(v_f4Color.xyz * cosTheta);
        f3SpecularCol += u_f3Specular * u_f3LightSpecular[i] * pow(cosAlpha, u_fShininess);
    }
